...
boutonVar1.addEventListener('click', function() {
    p1.innerHTML = 'Une variable est un conteneur ou un espace de stockage temporaire qui va pouvoir stocker une valeur. Lorsqu’on stocke une valeur dans une variable, on dit également qu’on assigne une valeur à une variable.';
});
...
boutonVar4.addEventListener('click', function() {
    p1.innerHTML = 'Sa principale caractéristique est sa portée : elle est limité à celle du bloc courant. Pour rappel, un bloc en Javascript, c’est ce qu’on retrouve entre accolades : une comparaison en if, une boucle while etc… Comme c’est une variable, on peut modifier sa valeur après sa déclaration.';
});
