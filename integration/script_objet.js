...
boutonObj2.addEventListener('click', function() {
    p4.innerHTML = `<br><br><br>L'objet global Array est utilisé pour créer des tableaux. Les tableaux sont des objets de haut-niveau (en termes de complexité homme-machine) semblables à des listes.<br>
    ...
    <br>
    En JavaScript, comme dans la plupart des langages de programmation, le premier élément du tableau commence à l'indice 0.
    `;
});

boutonObj4.addEventListener('click', function() {
    p4.innerHTML = `<br><br><br>La méthode JSON.parse() analyse une chaîne de caractères JSON et construit la valeur JavaScript ou l'objet décrit par cette chaîne. On peut éventuellement utiliser cette fonction avec un paramètre de modification permettant de traiter l'objet avant qu'il soit renvoyé.<br>
